...
def save_compressed_dictionary(destination, dictionary, filename):
    """compress a dictionary and save it to the disk"""
    ...
    # try saving the data
    try:
        np.savez_compressed(temp_name, **dictionary)
    except:
        print("ERROR: saving the compressed data to disk failed")
        return False

    # try reloading the data to ensure it was correctly saved
    try:
        with np.load(temp_name) as npzfile:
            reloaded = dict(zip(npzfile.files, [npzfile[x] for x in npzfile.files]))
    except:
        print("ERROR: saving the compressed data to disk failed (impossible to reload it")
        return False
...
def generate_sensorimotor_data(agent, environment, explo_type, k, dest_data="dataset", disp=True):
    """
    Generates a sensorimotor dataset and save it in <dest_data>/dataset_<explo_type>.npz.
    ...
    Output:
        The generated dataset is saved in <dest_data>/dataset_<explo_type>.npz as a dictionary with the following structure:
    ...
    if check_savability(dest_data, transitions) is False:
        print("Warning: the dataset would be too large to pickle - you could reduce the number of transitions, sensations, or motors")
    ...
    save_compressed_dictionary(dest_data, transitions, "dataset_{}.npz".format(explo_type))
...
def display_samples(dir_data, run_index, explo_type, n=24):
    ...
    data_file = data_directory + "/dataset_" + explo_type + ".npz"

    # load data
    transitions = load_compressed_sensorimotor_transitions(data_file)
