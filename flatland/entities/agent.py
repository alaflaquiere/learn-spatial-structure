...
from flatland.sensors.rgb_sensor import RgbSensor
from flatland.sensors.rgbFog_sensor import RgbFogSensor
from flatland.sensors.topview_sensor import TopviewSensor
from flatland.sensors.smell_sensor import SmellSensor
from flatland.sensors.overview_sensor import OverviewSensor

from flatland.entities.entity import Entity

from flatland.utils.pygame_util import to_pygame
...
class Agent(Entity):
    ...
    def __set_head(self, d_head):
        
        #print( self.__head )
        #print( d_head )
        
        if  self.__head + d_head > self.max_head:
            self.__head = self.max_head
        elif self.__head + d_head < -self.max_head:
            self.__head = - self.max_head
        else:
            self.__head = (self.__head + d_head)
            #self.__head = (self.__head + d_head)%(2*math.pi)
        #self.__had = d_head
    ...
    def apply_action(self, actions):
        ...
        head_angle = actions.get('head_angle', 0) 

        self.__set_head( head_angle )
