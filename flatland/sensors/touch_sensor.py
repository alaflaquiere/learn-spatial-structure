from flatland.sensors.sensor import Sensor
...
class TouchSensor(Sensor):
    ...
    def get_sensory_input(self, env):
        ...
        range_touch = env.agent.radius + 10         
                    
        polar_img = cv2.linearPolar(image, (agent_x, agent_y), range_touch , flags=cv2.INTER_NEAREST )
        angle_center = h * (( agent_angle ) % (2*math.pi))/ (2*math.pi) 
        rolled_img = np.roll(polar_img, int(h/2 - angle_center), axis = 0 )
        resized_img = cv2.resize( rolled_img, (rolled_img.shape[1], int(self.fovResolution)), interpolation = cv2.INTER_NEAREST )
